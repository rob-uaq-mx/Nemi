...
public:
    ...
    // Executes one line of source against a persistent session Environment
    // that the caller owns and reuses across calls — unlike evaluate() (a
    // fresh, throwaway frame every time), a variable assigned in one call
    // (`r ← factorial(5)`) remains visible to later ones. Tries the line as
    // a bare expression first (so a plain call or variable reference, e.g.
    // `factorial(5)` or `r`, still returns a printable value exactly like
    // run_call); if that doesn't parse or doesn't consume the whole line
    // (as with an assignment, where `←` isn't a valid expression operator),
    // falls back to parsing it as a single instrucción (statement) executed
    // for effect, returning monostate. Meant for an interactive console —
    // the plain CLI/run_call path is unaffected.
    Value run_line(const std::string& source, Environment& session);
